...
Runtime correctness for plugin loading and execution: `preAnalyze` loading effective plugins, accurate `applied`/`skipped` status for `beforeReportWrite` hooks, error handling in `resolveSchemaTypeOverride`, unsupported `apiVersion` warnings, and `disableBuiltins` option for the loader.
