...
Diff report owns the generation-path `useHistory` seam (`load → missing-report warn → apply`) so Generation item session prepares a Client through one history module instead of private load/apply wrappers.
