...
Versioned config schema support for plugin entries as strings or `{ path, name?, config? }` objects, `disableBuiltinPlugins` and `strictPluginMode` options, the `mergePluginPaths` core helper, and `check-config` file-existence validation.
