...
После `combineAndWrite`, при `rawOptions.trafficSplitter?.enabled === true`, ДОЛЖНА вызываться `generateTrafficSplitterModule` с output-папкой первого item. При `items.length > 1` ДОЛЖНО логироваться предупреждение, но генерация не прерывается.
