...
CLI flags `--plugins` and `--strict-plugin-mode` for the `analyze-diff` command, plugin path resolution merging config and CLI sources, and schema validation for the new fields.
