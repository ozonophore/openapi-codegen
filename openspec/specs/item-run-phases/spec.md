...
Free functions in `src/core/itemRun/` that implement one generation-item lifecycle phase each, so `GenerationItemSession.run()` can orchestrate without embedding those phases.
