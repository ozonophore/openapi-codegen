...
- Leftover: склейка файла после парсера — общий `joinTreeRefFile` (`file://` + `new URL`, не `path.resolve`). Его зовут `RefLookup` и expand. Expand не импортирует класс `RefLookup`. `isUrlLike` только `http` / `https` / `file`. Intern не расширять. Gate: оба снапшота `v3.withDifferentRefs` на Windows CI.
