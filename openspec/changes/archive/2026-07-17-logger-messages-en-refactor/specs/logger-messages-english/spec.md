...
### Requirement: Hardcoded Russian logger strings extracted from OpenApiClient
`src/core/OpenApiClient.ts` SHALL NOT contain inline Russian string literals passed to `logger.warn`. These strings MUST be moved to `LOGGER_MESSAGES.GENERATION.*` keys.
...
### Requirement: Hardcoded Russian CLI help strings translated inline
`src/cli/index.ts` SHALL NOT contain inline Russian string literals for commander option descriptions. Descriptions MUST be plain English string literals at the point of use — they MUST NOT be sourced from `LOGGER_MESSAGES`.
...
### Requirement: Hardcoded Russian dialog labels translated inline
`src/cli/checkAndUpdateConfig/constants.ts` SHALL NOT contain inline Russian string literals for interactive dialog choices. Labels MUST be plain English string literals at the point of use — they MUST NOT be sourced from `LOGGER_MESSAGES`.
