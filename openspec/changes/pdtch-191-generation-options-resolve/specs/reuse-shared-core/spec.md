...
Генератор ДОЛЖЕН вычислять эффективный `request` каждого item как после `resolveGenerationOptions` (`item.request ?? root.request`). Core-файлы, чьё содержимое зависит от custom request / executor / связанных флагов шаблона, ДОЛЖНЫ шариться только между items с одинаковым transport fingerprint. Fingerprint ДОЛЖЕН включать как минимум: эффективный `request` (или generated-default), `customExecutorPath` (или отсутствие), `httpClient`, `useCancelableRequest`, и факт экспорта `requestRaw` custom request при его наличии.
