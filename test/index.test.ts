...
describe('@unit: generate', () => {
    ...
    test('v3: modelsMode=classes generates models.ts and BaseDto', async () => {
        ...
        const rootIndex = path.join(process.cwd(), 'test', 'generated', 'v3-classes', 'index.ts');

        const modelsContent = readFileSync(modelsFile, 'utf8');
        const baseDtoContent = readFileSync(coreBaseDto, 'utf8');
        const indexContent = readFileSync(rootIndex, 'utf8');
        ...
        assert.match(indexContent, /export \* from '\.\/\.\/models\/models';/, 'Expected bundle classes export in root index.ts');
    });
