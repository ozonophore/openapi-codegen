...
### 8) Дефолтные пути CLI-отчётов (`2.1.0-beta.11`)
...
### 9) Набор возможностей Marauder (`2.1.0-beta.11`)

Preview-возможности «Marauder» поставляются в **`2.1.0-beta.11`**. Для ключей конфигурации они **аддитивные и опциональные**; всё выключено по умолчанию.

#### Схема конфигурации V6 (авто-миграция)

Актуальная унифицированная схема конфигурации теперь `UNIFIED_OPTIONS_v6`. Выполните:
...
| Стратегия | Поведение |
|-----------|-----------|
| `entity` | Per-output `.openapi-codegen-cache.json`; пропуск полной регенерации при неизменных входах (дефолт V5 после миграции) |
| `reuse` | Глобальный `.openapi-codegen-store` с общими артефактами model/schema, копируемыми в output (preview-дефолт в V6) |
...
- [ ] Выполнен `update-config` для миграции конфига на V6 (блоки Marauder).
