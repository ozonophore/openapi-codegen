...
- v2-хуки используются только в потоке `analyze-diff`.

## Унификация runtime через v3

v2-объекты остаются допустимым форматом экспорта, но загрузчик нормализует и v1, и v2 плагины в общий runtime-контракт v3 (`apiVersion: '3'`). Для новых плагинов рекомендуется v3 factory API из [Плагины](plugins.md#общий-контракт-плагина-v3-factory-api).
...
`analyze-diff` выполняет хуки строго по порядку плагинов в `plugins` из `openapi.config.json`:
...
- Два режима:
  - `strictPluginMode=false` (по умолчанию): ошибка хука логируется, выполнение продолжается.
  - `strictPluginMode=true` (флаг CLI `--strict-plugin-mode`): ошибка любого хука завершает команду с ошибкой.

## Подключение в analyze-diff

Пути к плагинам задаются в `openapi.config.json`:
...
Команда читает их через `--openapi-config` (по умолчанию `openapi.config.json`).

Дополнительные флаги CLI:

- `--strict-plugin-mode` — строгий режим обработки ошибок плагинов
- `--governance-config` — путь к JSON с правилами governance
- `--ci` — код выхода 1 при ошибках governance
- `--allow-breaking` — разрешить breaking changes в governance
