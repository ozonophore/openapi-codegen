...
describe('@unit: loadGovernanceConfig', () => {
    ...
    test('loads valid governance config', async () => {
        const tempDir = mkdtempSync(path.join(process.cwd(), 'test', 'generated', 'governance-config-valid-'));
    ...
    test('throws with path details for invalid severity', async () => {
        const tempDir = mkdtempSync(path.join(process.cwd(), 'test', 'generated', 'governance-config-invalid-severity-'));
        const configPath = path.join(tempDir, 'governance.json');
    ...
    test('throws with path details for invalid allowList item', async () => {
        const tempDir = mkdtempSync(path.join(process.cwd(), 'test', 'generated', 'governance-config-invalid-allowlist-'));
        const configPath = path.join(tempDir, 'governance.json');
