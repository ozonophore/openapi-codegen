...
test('@unit: migrateDataToLatestSchemaVersion (boolean Marauder shorthand → object blocks)', () => {
    ...
    assert.deepStrictEqual(result?.value.anomalyExploitation, { enabled: true });
});

test('@unit: migrateDataToLatestSchemaVersion (exploitAnomalies alias → anomalyExploitation)', () => {
    const rawInput = {
        input: './spec.json',
        output: './dist',
        httpClient: 'fetch',
