...
describe('@unit: parseNestedCliOptions', () => {
    ...
    test('leaves boolean shorthand flags for Commander', () => {
        const { cleanedArgv } = parseNestedCliOptions([...baseArgv, 'generate', '--auto-select', '--exploit-anomalies']);

        assert.deepStrictEqual(cleanedArgv, [...baseArgv, 'generate', '--auto-select', '--exploit-anomalies']);
    });
