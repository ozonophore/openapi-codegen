...
export class ServiceRule implements Rule {
  async check(
    ...
    for (const file of context.getConsumerSourceFiles()) {
      ...
      for (const call of calls) {
        ...
        const methodContract = contract.services[serviceName].find(
          (m) => m.name === methodName,
        );
        if (!methodContract) {
        ...
        const methodDecl =
          serviceDecl && Node.isClassDeclaration(serviceDecl)
        ...
        for (let i = 0; i < Math.min(args.length, methodContract.params.length); i++) {
          const expectedType = this.getExpectedParameterType(
            contract,
            serviceName,
            methodName,
            i,
  ...
  private getExpectedParameterType(
    ...
    const exportedDecl = contract.sourceFile
      ...
      ?.find((d) => Node.isClassDeclaration(d));
    const method =
      exportedDecl && Node.isClassDeclaration(exportedDecl)
